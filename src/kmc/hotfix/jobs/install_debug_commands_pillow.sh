if [ -f "/usr/share/webkit-1.0/pillow/debug_cmds.json" ] ; then
	...
	if ! grep -q "KMCLog.sh" "/usr/share/webkit-1.0/pillow/debug_cmds.json" ; then
		logmsg "I" "install_log_kpp" "" "Patching in the KMC log command for kpp"
        sed -e '/^{/a\' -e '    ";kmclog" : "/var/local/kmc/KMCLog.sh",' -i "/usr/share/webkit-1.0/pillow/debug_cmds.json"
	fi
fi